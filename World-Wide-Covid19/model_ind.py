...
class SEIRD:
      ...
      '''
        ...
        for t in range(7):
            ...
            inf_predicted.append(
                preds[best_param][len(preds[best_param])-7:][t][1])
            death_predicted.append(
                preds[best_param][len(preds[best_param])-7:][t][3])
    ...
    def final_run(self):
        ...
        # final_d = []
        preds = []
        _final_i, _final_d, best_param, preds = self.param_selection()
        inf_df = pd.DataFrame(_final_i, index=['Date', 'Active'])
        death_df = pd.DataFrame(_final_d, index=['Date', 'Deceased'])
        inf_df = inf_df.T
        death_df = death_df.T
        inf_df.to_csv(self.outfile + '_infected.csv')
        death_df.to_csv(self.outfile + '_deaths.csv')


infile = './Datasets/India/Maharashtra/Mumbai.csv'
# infile = 'Thane.csv'

# N = 12537954
# # N = 9426959  # pune
N = 11060148  # thane
model = SEIRD(infile, N, "hih")
