...
def feed_data(self):
        # try :
        df = pd.read_csv(self.infile, header=0)  # dataset
        # except :
        # print("----")
        # return False
...
def seird_model(self, str_dates, init_vals, params, t):
  ...
  for tic in t[1:]:
    ...
    next_S = S[-1] - (beta * (S[-1] / self.N) * I[-1])  # Susceptible
    # Exposed (Undetected + asymptomatic)
    next_E = E[-1] + (beta * (S[-1] / self.N) * I[-1]) - (alpha * E[-1])
    next_I = I[-1] + (alpha * E[-1]) - (gamma * I[-1]) - (delta * I[-1])  # Hospitalized
...
def param_estimator(self, str_dates, inf_train, rec_train, death_train):
        t2 = np.arange(0, 3, 1)
        #print("t2", t2)
        last5_vals = []
        last5_params = []
...
def predictions(self, last5_params, last5_vals):
        ...
        for z in range(1, len(last5_params)):#4params
            ...
            results = self.seird_model(str_dates3, last5_vals[z-1], last5_params[z-1], t7)
            #t_z = np.arange(7 - z, 14 - z, 1)
            t_z = np.arange(0, 15 - z, 1)
            temp = []
            for day in t_z:
                temp.append([ results[day][0], int(float(results[day][3])), int(float(results[day][4])), int(float(results[day][5])) ])
                #print(temp)
            preds.append(temp)
            #print("z", z)
        return preds
...
def param_selection(self):
  ...
  recovered = []
  deaths = []
  start_date, available, infected, recovered, deaths = feed_data(self)
  ...
  inf_train, rec_train, death_train = split_data(pred_start, infected, recovered, deaths)
  #print(rec_train)
  #print(death_train)
  ...
  last5_params, last5_vals = param_estimator(str_dates, inf_train, rec_train, death_train)
  #print("params", last5_params)
  #print("vals", last5_vals)


  preds = []
  preds = predictions(last5_params, last5_vals)
  ...
  for t in range(7):
    ...
    dates_p.append(preds[best_param][len(preds[best_param])-7:][t][0])
    inf_predicted.append(preds[best_param][len(preds[best_param])-7:][t][1])
    death_predicted.append(preds[best_param][len(preds[best_param])-7:][t][3])
...
def final_run(self):
  ...
  final_d = []
  preds = []
  final_i, final_d, best_param, preds = param_selection(self)

  out = pd.DataFrame(preds[best_param][len(preds[best_param])-7:], columns=["Date", "Active", "Recovered", "Deceased"])
  out.drop(['Recovered'], axis=1, inplace=True)
  predicted_i = out['Active'].values.tolist()
  predicted_d = out['Deceased'].values.tolist()
  #print(out)
  out.to_csv(self.infile[:-4] + "_projections.csv")
